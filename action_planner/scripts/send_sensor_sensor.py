...
class ActionNodeExample(ActionExecutorBase):
    ...
    def __init__(self):
        super().__init__("send_sensor_sensor")
    ...
    def new_goal(self, goal_request) -> bool:
        ...
        target = goal_request.parameters[1]
        location = goal_request.parameters[2]
        self.get_logger().info(f"SendSensorSensor: From {origin} To {target}. Location: {location}")
